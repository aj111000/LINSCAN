...
double kl_dist(double x[11], double y[11]) {
    ...
    initialize_array(diff, (double[2]) {
        x[0] - y[0], x[1] - y[1]
    }, 2, 1);

    double ** cov1 = create_array(2, 2);
    initialize_array(cov1, (double[4]) {
        x[2], x[3], x[3], x[4]
    }, 2, 2);

    double ** inv1 = create_array(2, 2);
    initialize_array(inv1, (double[4]) {
        x[5], x[6], x[6], x[7]
    }, 2, 2);

    double ** inv_sqrt1 = create_array(2, 2);
    initialize_array(inv_sqrt1, (double[4]) {
        x[8], x[9], x[9], x[10]
    }, 2, 2);

    double ** cov2 = create_array(2, 2);
    initialize_array(cov2, (double[4]) {
        y[2], y[3], y[3], y[4]
    }, 2, 2);

    double ** inv2 = create_array(2, 2);
    initialize_array(inv2, (double[4]) {
        y[5], y[6], y[6], y[7]
    }, 2, 2);

    double ** inv_sqrt2 = create_array(2, 2);
    initialize_array(inv_sqrt2, (double[4]) {
        y[8], y[9], y[9], y[10]
    }, 2, 2);

    double ** I = create_array(2, 2);
    initialize_array(I, (double[4]) {
        1,
        0,
        0,
        1
